...
def generate_answer(context: str, question: str) -> str: # str tra ve la cau tra loi
    if not context or not context.strip(): # Kiem tra context rong
        logger.warning("Received empty context for answer generation.")
        return "Dữ liệu ngữ cảnh không được để trống."
        
    if not question or not question.strip(): # Kiem tra cau hoi rong
